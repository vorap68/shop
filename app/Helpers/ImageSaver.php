...
class ImageSaver {
   ...
   public function upload($request, $item, $dir) {
       $name = $item->image ?? null;//это имя картинки  с БД
       ...
       if($source){ // если было загружено изображение
           ...
           if ($item && $item->image)  $this->remove($item, $dir);
           
       $ext = $source->extension();
            // сохраняем загруженное изображение без всяких изменений методом store
           ...
           // $path = $source->store('catalog/'.$dir.'/source','public');
            ...
            $path = Storage::disk('public')->path($path); // абсолютный путь
             //dd($path);
//             if($source) $name = $request->file('image')->getClientOriginalName();
    ...
    private function resize($src, $dst, $width, $height, $ext) {
        ...
        $image = Image::make($src)
            ...
            ->encode($ext, 100);
        // сохраняем это изображение под тем же именем, что исходное используя фасад и Storage
        // и его метод put()
        $name = basename($src);
        //dd($name);
